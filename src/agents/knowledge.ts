...
- BUSINESS DATA (variables) are instance-specific values such as timestamps, numbers,
  user or device identifiers, names, IP or MAC addresses, counters, IDs, and JSON
  payloads. They come from unbounded domains and replacing them does not change
  ...
  Note2: When the same type of BUSINESS DATA appears multiple times in a single log entry, use numbered variable names to avoid duplication (e.g., "ip1", "ip2", "mac1", "mac2").
...
- STRUCTURE (constants) are system-defined tokens such as event skeletons,
  log levels, module names, protocol keywords, message, and
  syntactic separators (colons, brackets, pipes). They draw from finite sets
  and altering them would change what the log entry represents.

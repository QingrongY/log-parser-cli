...
export const UPDATE_RESPONSE_SCHEMA: Record<string, unknown> = {
  type: 'object',
  required: ['action', 'correct template'],
  additionalProperties: false,
  properties: {
    action: { type: 'string', enum: ['Modify candidate', 'Modify existing'] },
    'correct template': { type: 'string', minLength: 1 },
    explain: { type: 'string' },
  },
};
...
export const buildUpdatePrompt = ({
  ...
  return [
    ...
    'A new candidate regex matches logs belong to existing templates.',
    'Strictly following the Shared Background Knowledge,',
    'decide whether to modify the candidate or the existing templates to ensure the library stays mutually exclusive',
    ...
    '   The existing templates are too strict and incorrectly treats BUSINESS VARIABLES as constants.',
    ...
    '  "correct template": "Modified candidate template" | "Modified existing template",',
    '  "BUSINESS DATA": {',
    '    "name1": "value1",',
    '    "name2": "value2",',
    '    ...',
